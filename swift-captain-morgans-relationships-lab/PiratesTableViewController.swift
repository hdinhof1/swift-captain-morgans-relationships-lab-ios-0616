...
class PiratesTableViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.detailTextLabel?.text = "\(store.pirates[indexPath.row].ships!.count)"
    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        let destinationShipVC = segue.destinationViewController as! ShipTableViewController
        
        let indexPath = tableView.indexPathForSelectedRow?.row
        
        
        if let currentRow = indexPath, let shipsSet = store.pirates[currentRow].ships {
            destinationShipVC.ships = shipsSet
        }
